        ...
        const loadData = async () => {
            if (activeTab === 'overview' || activeTab === 'clinics' || activeTab === 'approvals') {
                try {
                    ...
                    logger.log("[SuperAdminDashboard] All clinics loaded:", data.length, "clinics");
                    logger.log("[SuperAdminDashboard] Clinic statuses:", data.map(c => ({
                        name: c.name,
                        status: c.status,
                    ...
                    const clinicRequests = data.map(c => {
                        ...
                        return {
                            ...
                            date: c.joinedDate,
                            details: `Plan: ${c.plan}, Email: ${c.email}${c.phone ? `, Phone: ${c.phone}` : ''}${c.location ? `, Loc: ${c.location}` : ''}`,
                            status: reqStatus
                        };
                    });

                    logger.log("[SuperAdminDashboard] Clinic requests created:", clinicRequests.length);
                    setRequests(clinicRequests);

                } catch (error) {
                    logger.error("Failed to load clinics", error);
                }
            }
            ...
            if (activeTab === 'overview' || activeTab === 'settings') {
                try {
                    ...
                    logger.error("Failed to load system logs", error);
                }
            }
        ...
        try {
            ...
            if (newClinicForm.email) {
                try {
                    ...
                    logger.error("Failed to auto-assign admin during provision:", assignError);
