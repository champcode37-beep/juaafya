...
const Appointments: React.FC<AppointmentsProps> = ({
  ...
  return (
    <div
      className="flex-1 w-full max-w-[1600px] mx-auto p-4 sm:p-6 md:p-8 bg-gray-50/50 dark:bg-slate-900/50 min-h-screen transition-all duration-300"
      onClick={() => {
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div className="text-center sm:text-left">
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Appointments</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage your schedule and patient visits</p>
        </div>

        <div className="flex items-center justify-center sm:justify-end gap-2 sm:gap-3 flex-wrap">
          {/* Search */}
          <div className="relative flex-1 group sm:max-w-xs">
            ...
            <input
              ...
              className="pl-9 pr-8 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl text-xs outline-none focus:ring-4 focus:ring-brand-blue/10 w-full dark:text-white shadow-sm transition-all"
            />
            {searchTerm && (
              <button
                onClick={() => setSearchTerm("")}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 active:scale-90"
              >
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-2 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-200 px-3 sm:px-4 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all text-xs font-bold active:scale-95 shadow-sm ${dateFilter !== "all" ? "ring-2 ring-brand-blue" : ""}`}
            >
            ...
            {isFilterOpen && (
              <div className="absolute top-full right-0 mt-2 w-40 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-white/5 z-[60] overflow-hidden animate-in fade-in zoom-in-95">
                {[
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-xs font-bold transition-colors ${dateFilter === option.value ? "text-brand-blue bg-brand-blue/5" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5"}`}
                  >
                    {option.label}
                  </button>
                ))}
          ...
          <button
            onClick={() => {
              if (!canExport) {
                useStore.getState().actions.showToast('No permission to export.', 'error')
                return
              }
              handleExportCSV()
            }}
            disabled={!canExport}
            className={`p-2.5 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl border border-slate-200 dark:border-white/10 transition-all active:scale-95 shadow-sm ${canExport ? 'hover:bg-slate-50' : 'opacity-40 cursor-not-allowed'}`}
          >
      ...
      <div className="flex items-center gap-6 border-b border-slate-200 dark:border-white/10 mb-6 overflow-x-auto scrollbar-hide">
        {(["upcoming", "completed", "cancelled"] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`pb-3 text-xs sm:text-sm font-black uppercase tracking-wider transition-all relative flex items-center gap-2 shrink-0 ${activeTab === tab
              ? "text-brand-blue"
          ...
          >
            {tab}
            <span
              className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${activeTab === tab ? "bg-brand-blue/10 text-brand-blue" : "bg-slate-100 dark:bg-white/5 text-slate-500"}`}
            >
              {tabCounts[tab]}
            </span>
            {activeTab === tab && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-brand-blue rounded-t-full shadow-[0_-4px_10px_rgba(52,98,238,0.3)]"></div>
            )}
          </button>
        ))}
      </div>

      <div className="card-elegant overflow-hidden transition-all">
          ...
          <div className="divide-y divide-slate-50 dark:divide-white/5">
            {filteredAppointments.map((appt) => {
              ...
              return (
                <div
                  key={appt.id}
                  className="p-4 sm:p-6 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
                >
                  <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
                    <div className="flex flex-col items-center justify-center w-12 h-12 sm:w-14 sm:h-14 bg-brand-blue/5 dark:bg-brand-blue/10 text-brand-blue rounded-xl sm:rounded-2xl border border-brand-blue/10 shrink-0">
                      <span className="text-[9px] sm:text-xs font-black uppercase tracking-tighter">
                        {new Date(appt.date).toLocaleString("default", { month: "short" })}
                      </span>
                      <span className="text-lg sm:text-xl font-black leading-none">{new Date(appt.date).getDate()}</span>
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="text-sm sm:text-lg font-black text-slate-900 dark:text-white truncate">{appt.patientName}</h3>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-slate-400 mt-1">
                        <div className="flex items-center gap-1 font-bold">
                          <Clock className="w-3.5 h-3.5" />
                          {appt.time}
                        </div>
                        <span className="hidden xs:block w-1 h-1 rounded-full bg-slate-300 dark:bg-white/10 text-center"></span>
                        <span className="truncate max-w-[150px] font-medium italic">{appt.reason}</span>
                      </div>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 w-full sm:w-auto mt-1 sm:mt-0 overflow-x-auto scrollbar-hide pb-1 sm:pb-0">
