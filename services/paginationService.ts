...
export const paginationService = {
  ...
  async getPaginatedPatients(params: PaginationParams): Promise<PaginatedResponse<any>> {
    try {
      ...
      let query = supabase
        .from("patients")
        .select("id, full_name, phone_number, date_of_birth, gender, updated_at, history, vitals", { count: "exact" })
      ...
      console.error("Error fetching paginated patients:", error)
      return { data: [], total: 0, page: params.page, pageSize: params.pageSize, totalPages: 0, hasMore: false }
    }
  },
